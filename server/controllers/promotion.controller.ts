...
export async function getPromotions(req: Request, res: Response) {
  try {
    const limit = req.query.limit ? Number(req.query.limit) : undefined;
    const promotions = await storage.getPromotions(limit);

    // Processar as promoções para garantir URLs de imagem corretas
    const processedPromotions = promotions.map(promotion => {
      const product = promotion.product;
      if (product) {
        // Se o produto tem imagens
        if (Array.isArray(product.images)) {
          // Processar cada imagem para garantir URL direta correta
          product.images = product.images.map((img: any) => {
            if (typeof img === 'object') {
              // Se é um objeto com image_url
              let imageUrl = img.image_url;
              if (imageUrl.includes('/api/products/')) {
                // Converter para URL direta
                imageUrl = `/uploads/stores/${product.store?.id}/products/${product.id}/image.jpg`;
              }
              return {
                ...img,
                image_url: imageUrl
            ...
            if (typeof img === 'string') {
              if (img.includes('/api/products/')) {
                // Converter para URL direta
                return `/uploads/stores/${product.store?.id}/products/${product.id}/image.jpg`;
              }
              return img;
            }
            return img;
          ...
          // Definir imageUrl principal
          const primaryImage = product.images.find((img: any) => 
            typeof img === 'object' && img.is_primary
          );
          if (primaryImage) {
            product.imageUrl = typeof primaryImage === 'object' ? primaryImage.image_url : primaryImage;
          } else if (product.images.length > 0) {
            product.imageUrl = typeof product.images[0] === 'object' 
